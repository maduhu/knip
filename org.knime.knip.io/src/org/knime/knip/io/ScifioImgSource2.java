 ...
 * Gabriel Einsdorf
 */
public class ScifioImgSource2 implements ImgSource {

	private static final NodeLogger LOGGER = NodeLogger.getLogger(ScifioImgSource2.class);

	/* ID of the source */
	private static final String SOURCE_ID = "Scifio Image Source 2";
	...
	private final ImgOpener imgOpener;

	@SuppressWarnings("rawtypes")
	private final ImgFactory imgFactory;

	private ImgUtilityService imgUtilsService;
	...
	public ScifioImgSource2() {
		this(true);
	}

	@SuppressWarnings("rawtypes")
	public ScifioImgSource2(final boolean checkFileFormat) {
	...
	public ScifioImgSource2(@SuppressWarnings("rawtypes") final ImgFactory imgFactory, final boolean checkFileFormat,
		...
		this.usedDifferentReaders = false;
	...
	public ImgPlus<RealType> getImg(final String imgRef, final int currentSeries,
		...
		if (imgFactory instanceof CellImgFactory) {
			...
			// register
			final Context ctx = imgOpener.getContext();
		...
		final ImgPlus<RealType> ret = MiscViews.cleanImgPlus(
				imgOpener.openImg(getReader(imgRef), getPixelType(imgRef, currentSeries), imgFactory, options));
	...
	public RealType getPixelType(final String imgRef, final int currentSeries) throws IOException, FormatException {
		...
		if (imgUtilsService == null) {
			imgUtilsService = ScifioGateway.getSCIFIO().getContext().getService(ImgUtilityService.class);
		}

		final RealType type = imgUtilsService
	...
	private UnclosableReaderFilter getReader(final String imgRef) throws FormatException, IOException {
		org.apache.log4j.Logger.getLogger(KNIPLogService.class.getSimpleName()).setLevel(Level.ERROR);
		if (reader == null || (!currentFile.equals(imgRef) && checkFileFormat)) {
			...
			r.setMetadata(p.parse(imgRef, scifioConfig));
			...
			reader = r;
		} else if (!currentFile.equals(imgRef) && !checkFileFormat) {
			...
			reader.setMetadata(p.parse(imgRef, scifioConfig));
	...
	/**
	 * Reads all images referenced by this uri
	 * 
	 * @param uri
	 */
	public <T extends RealType<T> & NativeType<T>> List<ImgPlus<T>> readImgs(URI uri) {

		ImgOpener op = new ImgOpener();
		// op.openImgs(source, imgFactory, config)
		// TODO Auto-generated method stub
		return null;
	}

	public <T extends RealType<T> & NativeType<T>> List<ImgPlus<T>> readImgs(URI uri, ConnectionInformation info) {
